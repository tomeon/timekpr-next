...
PLAYTIME_FIELDS = {
    ...
    "allow_unaccounted_intervals": ("PLAYTIME_UNACCOUNTED_INTERVALS_ENABLED", "setPlayTimeUnaccountedIntervalsEnabled"),
}
...
def hours_from_daemon(hours):
    ...
    return [
        {"hour": int(hour), "start_minute": int(spec[cons.TK_CTRL_SMIN]), "end_minute": int(spec[cons.TK_CTRL_EMIN]), "unaccounted": bool(spec[cons.TK_CTRL_UACC])}
        for hour, spec in sorted(hours.items(), key=lambda item: int(item[0]))
    ]


def hours_to_daemon(entries):
    return {str(entry["hour"]): {cons.TK_CTRL_SMIN: entry.get("start_minute", 0), cons.TK_CTRL_EMIN: entry.get("end_minute", 60), cons.TK_CTRL_UACC: int(entry.get("unaccounted", False))} for entry in entries}
...
def user_config_from_daemon(info):
    ...
    config = {
        ...
        "playtime": {
            ...
            "activities": [{"process": activity[0], "description": activity[1]} for activity in info["PLAYTIME_ACTIVITIES"]],
        },
    }
    config.update({field: info[key] for field, (key, _setter) in USER_FIELDS.items()})
    config["playtime"].update({field: info[key] for field, (key, _setter) in PLAYTIME_FIELDS.items()})
    return config
...
def user_config_to_daemon(config):
    ...
    info["PLAYTIME_LIMITS_PER_WEEKDAYS"] = limits_list(playtime["allowed_days"], playtime["limits_per_day"])
    info["PLAYTIME_ACTIVITIES"] = [[activity["process"], activity["description"]] for activity in playtime["activities"]]
    return info
...
def user_status_from_daemon(info):
    ...
    if status["session_active"]:
        ...
        status.update({field: None for field in STATUS_LIVE if field not in STATUS_SAVED})
    return status
