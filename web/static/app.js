...
const SERVER_FIELDS = [
  ...
  ["playtime_enhanced_activity_monitor", "PlayTime enhanced activity monitor", "checkbox"],
];

const $ = (selector, root = document) => root.querySelector(selector);
const state = { token: sessionStorage.getItem("timekprw-token") || "", user: null, config: null, hours: {}, server: null };
...
function hoursToText(entries) {
  return entries.map((e) => `${e.unaccounted ? "!" : ""}${e.hour}${e.start_minute === 0 && e.end_minute === 60 ? "" : `[${e.start_minute}-${e.end_minute}]`}`).join(";");
}
...
function textToHours(text) {
  return text.split(";").map((s) => s.trim()).filter(Boolean).map((item) => {
    ...
    return { hour: Number(match[2]), start_minute: Number(match[3] ?? 0), end_minute: Number(match[4] ?? 60), unaccounted: match[1] === "!" };
  });
}
...
async function api(method, path, body) {
  ...
  if (!response.ok) {
    ...
    const errors = (data.errors || []).map((e) => `${e.field}: ${e.message}`).join("; ");
    throw new Error(`${data.title}${data.detail ? `: ${data.detail}` : ""}${errors ? ` (${errors})` : ""}`);
  }
  return data;
}
...
function showPage(name) {
  ...
  for (const button of document.querySelectorAll("nav button")) button.classList.toggle("active", button.id === `nav-${name}`);
...
async function refreshHealth() {
  ...
  try {
    ...
    el.textContent = health.daemon === "ok" ? `daemon ${health.timekpr_version}` : "daemon unreachable";
...
async function loadUsers() {
  ...
  list.replaceChildren(...users.map((user) => {
    ...
    $(".sub", li).textContent = `${user.full_name ? `${user.full_name} · ` : ""}${hms(user.status.time_left_day)} left today`;
...
async function selectUser(username) {
  state.user = username;
  for (const li of document.querySelectorAll("#user-list li")) li.classList.toggle("selected", $(".name", li).textContent === username);
...
function renderStatus(status) {
  ...
  $("#user-status").replaceChildren(...rows.flatMap(([label, value]) => {
...
function renderDayTable(tableId, prefix, allowedDays, limits) {
  $(`#${tableId} tbody`).replaceChildren(...DAYS.map((day) => {
    ...
    $(`[name=${prefix}allowed_${day}]`, tr).checked = allowedDays.includes(day);
...
function readDayTable(form, prefix) {
  const allowed_days = DAYS.filter((day) => form[`${prefix}allowed_${day}`].checked);
  const limits_per_day = {};
  for (const day of allowed_days) limits_per_day[day] = parseHms(form[`${prefix}limit_${day}`].value);
  return { allowed_days, limits_per_day };
}

function renderHoursGrid() {
  $("#hours-grid tbody").replaceChildren(...DAYS.map((day) => {
    const tr = document.createElement("tr");
    ...
    for (const hour of HOURS) {
      ...
      if (entry && (entry.start_minute !== 0 || entry.end_minute !== 60)) td.classList.add("partial");
...
function cycleHour(day, hour) {
  ...
  if (!current) entries.push({ hour, start_minute: 0, end_minute: 60, unaccounted: false });
  else if (!current.unaccounted) entries.push({ hour, start_minute: 0, end_minute: 60, unaccounted: true });
...
function renderConfig(config) {
  state.config = config;
  state.hours = Object.fromEntries(DAYS.map((day) => [day, config.allowed_hours[day].map((e) => ({ ...e }))]));
  const form = $("#config-form");
  renderDayTable("day-limits", "", config.allowed_days, config.limits_per_day);
  renderDayTable("pt-day-limits", "pt_", config.playtime.allowed_days, config.playtime.limits_per_day);
  renderHoursGrid();
  ...
  form.pt_allow_unaccounted_intervals.checked = config.playtime.allow_unaccounted_intervals;
  form.pt_activities.value = config.playtime.activities.map((a) => (a.description ? `${a.process} = ${a.description}` : a.process)).join("\n");
}
...
async function applyTimeLeft(event) {
  ...
  try {
    const body = { operation: form.operation.value, seconds: parseHms(form.amount.value) };
    const path = form.playtime.checked ? "playtime-left" : "time-left";
    renderStatus(await api("POST", `/users/${encode(state.user)}/${path}`, body));
...
async function loadServer() {
  ...
  $("#server-fields").replaceChildren(...SERVER_FIELDS.map(([field, label, kind]) => {
    const tr = document.createElement("tr");
...
async function saveServer(event) {
  ...
  for (const [field, , kind] of SERVER_FIELDS) {
    ...
    next[field] = kind === "checkbox" ? input.checked : kind === "list" ? input.value.split(";").map((s) => s.trim()).filter(Boolean) : Number(input.value);
  }
...
for (const name of ["users", "server", "token"]) $(`#nav-${name}`).addEventListener("click", () => showPage(name));
$("#config-form").addEventListener("submit", saveConfig);
$("#config-form").lockout_type.addEventListener("change", (event) => { $("#wake-hours").hidden = event.target.value !== "suspendwake"; });
$("#config-reload").addEventListener("click", () => loadUser().catch((err) => message(err.message)));
...
$("#token-form").addEventListener("submit", (event) => {
  ...
  showPage("users");
  loadUsers().then(() => message(""), (err) => message(err.message));
});

refreshHealth();
setInterval(refreshHealth, 30000);
loadUsers().catch((err) => message(err.message));
setInterval(() => { if (!$("#page-users").hidden) loadUsers().catch(() => {}); }, 15000);
