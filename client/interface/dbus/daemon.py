...
class timekprClient(object):
    ...
    def startTimekprClient(self):
        ...
        self._timekprClientIndicator = appind_timekprIndicator(
            self._userName, self._userNameFull, self._timekprClientConfig
        )

        # if not supported fall back to statico
        if not self._timekprClientIndicator.isSupported():
            # check if appind is supported
            self._timekprClientIndicator = statico_timekprIndicator(
                self._userName, self._userNameFull, self._timekprClientConfig
            )
        ...
        if self._timekprClientIndicator.isSupported():
            ...
            self._timekprClientIndicator.notifyUser(
                ...
                None,
                "cannot initialize the icon in any way",
            )
    ...
    def connectTimekprSignalsDBUS(self):
        ...
        try:
            ...
            # get dbus object
            self._notificationFromDBUS = self._timekprBus.get_object(
                cons.TK_DBUS_BUS_NAME,
                cons.TK_DBUS_USER_NOTIF_PATH_PREFIX + self._userNameDBUS,
            )
            ...
            log.log(
                cons.TK_LOG_LEVEL_INFO,
                "ERROR: failed to connect to timekpr dbus, trying again...",
            )
    ...
    def receiveTimeLeft(self, pPriority, pTimeInformation):
        ...
        # process time left
        self._timekprClientIndicator.setTimeLeft(
            pPriority,
    ...
    def receiveTimeLeftNotification(
        ...
        ) or pPriority == cons.TK_PRIO_CRITICAL:
    ...
    def receiveTimeCriticalNotification(
        ...
        self._timekprClientIndicator.notifyUser(
            cons.TK_MSG_CODE_TIMECRITICAL,
            pFinalNotificationType,
            pPriority,
            cons.TK_DATETIME_START + timedelta(seconds=pSecondsLeft),
        )
