...
The proposed model, in one paragraph: a _policy_ is a config file. A
...
something for that user, so its presence _is_ the "user policy
...
- `common/utils/config.py`: `loadUserConfiguration` no longer calls
  ...
  and must also notice a file _appearing_ while the user is logged in.
...
- NixOS test: the per-user "unrestricted login first" step
  ...
  by asserting that `--settimelimits` works _before_ first login and is
...
- In the default mode the daemon matches the configured mask against
  ...
  does _not_ defeat that. Copying the binary to another name, or
...
- `server/config/configprocessor.py`: the constructor takes a target;
  ...
  the _effective_ policy with provenance keys `POLICY_SOURCE` and
