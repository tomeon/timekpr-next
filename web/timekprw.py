...
def env_flag(name):
    ...
    if value in ("1", "true", "yes", "on"):
        return True
    raise SystemExit("timekprw: TIMEKPRW_%s must be true or false, not %r" % (name, value))
    ...
    for path in token_file_candidates(explicit):
        if os.path.exists(path):
            try:
                ...
                raise SystemExit("timekprw: cannot read token file %s; it must be readable by the service (0640 root:%s), or pass it as the systemd credential \"token\"" % (path, SOCKET_GROUP))
            if token == "":
                raise SystemExit("timekprw: token file %s is empty" % (path))
            if os.stat(path).st_mode & stat.S_IROTH:
                print("timekprw: WARNING: token file %s is readable by everyone; make it 0640 root:%s" % (path, SOCKET_GROUP), file=sys.stderr)
            return token
    return None
...
def parse_args(argv):
    ...
    parser.add_argument("--no-auth", action="store_true", default=env_flag("NO_AUTH"), help="serve TCP without authentication (only behind a reverse proxy that authenticates)")
    parser.add_argument("--auth-unix", action="store_true", default=env_flag("AUTH_UNIX"), help="require the bearer token on UNIX sockets too, instead of trusting whoever may open them")
    parser.add_argument("--allowed-host", action="append", metavar="HOST", default=env_default("ALLOWED_HOSTS", "").split() or None,
...
def main(argv=None, bridge=None):
    ...
    trusted = set() if args.auth_unix else {sock.getsockname() for sock in unix if isinstance(sock.getsockname(), str)}
    ...
    allowed_hosts.update(hostname(spec) for spec in (args.listen or []) if not spec.startswith(("unix:", "fd:")))
    ...
    app = create_app(bridge or Bridge(), static_dir=args.static_dir or None, token=token, allowed_hosts=allowed_hosts, trusted_sockets=trusted)
