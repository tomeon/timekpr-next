...
- `PATCH` on a user's or a group's config is one D-Bus call,
  `applyPolicyChanges`, which the daemon applies in one transaction of
  its policy database: every field or none. A refusal names the field
  in `errors`. `PATCH /api/v1/config` (the daemon-wide settings) is
  applied as a sequence of D-Bus setters, one per field, in a fixed
  order. The backend validates the whole body first. If a setter
  still fails part-way, the response is `400` with `errors` naming the
  failed field and `applied` listing the fields that were written
  before it.
...
| `PATCH`  | `/api/v1/groups/{group}/config`                     | the `--set*` commands with `@group` | Partial update, fields below. A `PATCH` creates the group's policy when there is none, even an empty one, so this is never `404`; `overrides` is set as `setOverrides` sets it, the rest as for a user's config. |
...
policy does not hold it); nulls are applied before values. The whole
`PATCH` is one `applyPolicyChanges` call, each field applied as the
D-Bus setter below applies it, and a request the daemon refuses
changes nothing. The fields are:

| Field             | Type                                                     | D-Bus setter                          | Notes                                                                                                                                                                                                                                                                                                                                                        |
| ----------------- | -------------------------------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `allowed_days`    | list of weekdays, e.g. `[1,2,3,4,5]`                     | `setAllowedDays`                      |                                                                                                                                                                                                                                                                                                                                                              |
| `limits_per_day`  | object weekday → seconds, `{"1": 7200, ..., "7": 10800}` | `setTimeLimitForDays`                 | The daemon stores limits positionally against `allowed_days` (`server/user/userdata.py`), so `GET` lists only allowed days, keys for other days are ignored, missing days keep their current value (a newly allowed day has `0`), and the daemon aligns the limits with the new days when `allowed_days` changes. Values are clamped to 86400 by the daemon. |
